...
| File | Description |
| :--- | :--- |
| creation.c` | C code demonstrating the `fork()` system call (Exercise 1). |
| `filealia.c`, `file2alia.c` | C code demonstrating the role of the Linker (Exercise 5). |
| `program.c` | C code for inspecting dynamic libraries with `ldd` (Exercise 6). |
| `filemaker` | Automates the compilation and execution of all C programs. |
